...
def add_watermark_to_image(image_path, watermark_image_path, output_path):
...
def upload_file():
    if request.method == 'POST':
        ...
        if image_file and allowed_file(image_file.filename):
            ...
            result_filename = add_watermark_to_image(image_path, watermark_path, image_path)

            # 업로드된 이미지와 워터마크 이미지 삭제
            os.remove(image_path)
            if not use_default_watermark:
